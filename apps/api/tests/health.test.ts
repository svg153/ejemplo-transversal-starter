...
jest.mock('../src/services/database-health', () => ({ // Mock the database probe so tests stay isolated from PostgreSQL.
  getDatabaseHealthStatus: jest.fn(), // Expose a controllable Jest mock for the route dependency.
})); // Finish the health service mock.

const mockedGetDatabaseHealthStatus = getDatabaseHealthStatus as jest.MockedFunction<typeof getDatabaseHealthStatus>; // Create a typed handle for the mocked health helper.

describe('GET /api/health', () => { // Cover the DB-aware health endpoint behavior.
  let server: Server; // Hold the per-test HTTP server instance.

  afterEach(async () => { // Close the server and reset mocks after each test.
  ...
  async function startServer(): Promise<number> { // Start the app and return the ephemeral port.
    server = createServer(app); // Boot the real Express app in-process.

    await new Promise<void>((resolve) => { // Wait for the server to bind a random free port.
  ...
  it('returns an ok payload when the database is connected', async () => { // Verify the healthy runtime case.
    ...
    await expect(response.json()).resolves.toEqual({ // Validate the full response payload.
  ...
  it('returns a degraded payload when the database is disconnected', async () => { // Verify the degraded runtime case.
