...
describe(`GET ${config.apiBasePath}/version`, () => {
  // Keep the test name aligned with the runtime route.
  let server: Server; // Hold the per-test HTTP server instance.

  afterEach(async () => {
    // Close the server after each test to avoid port leaks.
    if (!server) {
  ...
  it("returns the backend package metadata and api base path", async () => {
    // Verify the full stable payload.
    server = createServer(app); // Start the app through a real HTTP server.

    await new Promise<void>((resolve) => {
      // Wait for the server to bind an ephemeral port.
    ...
    const response = await fetch(
      // Call the version endpoint through HTTP.
    ...
    await expect(response.json()).resolves.toEqual({
      // Validate the complete response payload.
