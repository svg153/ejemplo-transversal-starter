...
export const appDataSource = new DataSource({ // Build the shared PostgreSQL data source instance.
...
export function initializeDatabase(): Promise<DataSource> { // Initialize the data source and run pending migrations once.
  if (appDataSource.isInitialized) { // Reuse the initialized data source when it is already ready.
    return Promise.resolve(appDataSource); // Resolve immediately with the shared data source.
  } // Finish the initialized guard.

  if (!initializationPromise) { // Start initialization only on the first caller.
    initializationPromise = appDataSource.initialize().then(async (dataSource) => { // Open the PostgreSQL connection lazily.
    ...
    }).catch((error: unknown) => { // Reset the cached promise when startup fails.
...
export async function closeDatabase(): Promise<void> { // Close the shared data source when tests or shutdown need it.
  initializationPromise = null; // Clear the cached initialization state before closing.

  if (!appDataSource.isInitialized) { // Skip teardown when the data source never initialized.
