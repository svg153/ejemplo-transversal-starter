...
export async function getDatabaseHealthStatus(): Promise<DatabaseHealthStatus> {
  // Probe the current database connection state for the health route.
  if (!appDataSource.isInitialized) {
    // Treat an uninitialized data source as unavailable.
    return "disconnected"; // Report the disconnected state explicitly.
  } // Finish the initialization guard.

  try {
    // Probe the current connection with a lightweight query.
    ...
    // Fall back to a disconnected state when the probe fails.
