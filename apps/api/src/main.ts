...
async function bootstrap(): Promise<void> {
  // Start the database and HTTP server in a controlled order.
  ...
  app.listen(config.port, config.host, () => {
    ...
    console.log(
      `API listening on http://${config.host}:${config.port}${config.apiBasePath}`,
    ); // Print the resolved local API URL.
  }); // Finish the HTTP server bootstrap.
} // Finish the bootstrap helper.

bootstrap().catch((error: unknown) => {
  // Fail fast when the backend cannot reach PostgreSQL.
