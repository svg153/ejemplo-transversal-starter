...
type HealthResponse = { // Define the stable health-check payload explicitly.
...
healthRouter.get(`${config.apiBasePath}/health`, async (_request, response) => { // Expose the health endpoint under the API base path.
  const database = await getDatabaseHealthStatus(); // Probe the database connection when the route is called.
  const payload: HealthResponse = { // Build the stable response payload from the probe result.
