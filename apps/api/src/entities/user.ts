...
export interface UserEntity { // Define the persisted shape for backend users.
...
export const UserEntitySchema = new EntitySchema<UserEntity>({ // Register the user table schema for TypeORM.
  ...
  columns: { // Describe each persisted column explicitly.
    ...
    passwordHash: { // Configure the password hash column.
